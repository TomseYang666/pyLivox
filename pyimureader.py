...
def imu_read(filepath):
    ...
    # 确定文件头的格式
    f = open(filepath, 'rb')
    imu_window = [b'\x57', b'\x48']
    imu_window[0] = f.read(1)
    imu_window[1] = f.read(1)
    offset = 0
    while True:
        if (imu_window[0] == b'\x57') & (imu_window[1] == b'\x48'):
            break
        data = f.read(1)
        offset += 1
        imu_window[0] = imu_window[1]
        imu_window[1] = data
    f.close()
    imu_header = '<' + str(offset) + 'x'
    imu_info = '<4x 33c 7x'
    # -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    # 读取文件头
    f = open(filepath, 'rb')
